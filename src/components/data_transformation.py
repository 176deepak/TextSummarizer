...
class DataTransformation:
    ...
    def convert_examples_to_features(self, data):
        ...
        labels = self.tokenizer(text_target=data[' summary'], max_length=128, truncation=True)
    ...
    def convert(self):
        ...
        dataset_BBC_pt = dataset_BBC.map(
            self.convert_examples_to_features, batched=True
        )
        dataset_BBC_pt.save_to_disk(os.path.join(self.config.root_dir, "BBC_data"))
